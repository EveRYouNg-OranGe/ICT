...
class TileLabel(QLabel):
    """A QLabel that can notify on double-click."""
...
class CompareViewer(QWidget):
    ...
    def _toggle_zoom(self):
        """Toggle between two zoom levels and refresh."""
    ...
    def _load_tile(self, path: Optional[Path], title: str, cell_w: int, cell_h: int, zoom_factor: float = 1.0) -> QPixmap:
        ...
        if path is None or (not path.exists()):
            ...
            if z <= 1.0:
                ...
                left = (zoomed.size[0] - cell_w) // 2
                top = (zoomed.size[1] - cell_h) // 2
    ...
    def _refresh(self):
        ...
        # Fill grid with enabled models (AUTO GRID)
        ...
        for m in enabled_models:
            ...
            pix = self._load_tile(path, m, cell_w, cell_h, self.zoom_factor)

            lab = TileLabel(on_double_click=self._toggle_zoom)
            lab.setFixedSize(int(cell_w), int(cell_h))
            lab.setScaledContents(True)  # 关键：pixmap 会随 label 拉伸铺满
            lab.setPixmap(pix)
            lab.setAlignment(Qt.AlignmentFlag.AlignCenter if QT6 else Qt.AlignCenter)
            # 关键：让 QLabel “按格子”占位，视觉上铺满
